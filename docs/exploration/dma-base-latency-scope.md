...
This is deterministic base/payload/command accounting, not a queue-aware memory controller. Payload beats may be packed per descriptor or aligned per logical segment, and command issue may be serialized with or ideally overlapped by payload; see `dma-payload-segment-alignment.md` and `dma-issue-payload-overlap.md`. The model still does not include TLB/page walks, source/destination address alignment, protocol boundaries, descriptor fetch, finite command credits, overlap bubbles, SRAM replay, DRAM bank timing, physical area/power, or calibration. `logical_segments` charges the configured aggregate base term per segment; it does not decompose which physical stages actually restart. Multicast fanout remains outside this scope because the current logical-segmentation contract does not classify destinations as independent segments.
