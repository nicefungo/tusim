...
- **FIXED** fires exactly at the schedule. **DEFERRED** fires at the earlier of (a) the first access to the bank after the schedule (that access pays the full duration) or (b) the hard deadline schedule + max_deferral (tick fires it even with no traffic). A deferred fire does **not** reset the schedule: the next nominal due time remains on the original tREFI grid (`refresh_next += tREFI`). The late command consumes slack rather than allowing repeated postponement to lower the long-term refresh rate; see `dram-deferred-refresh-debt.md`.
...
- **Deferred is phase-insensitive and always bad here:** 85,100–101,375 (+325% to +407%) at every phase. The nominal schedule remains on the original 7,800-cycle grid; every burst that arrives after a due point but before its deadline opportunistically triggers the outstanding refresh and pays the lockout. The workload's bursts recur every 5,050 cycles, so sub-tREFI idle gaps repeatedly expose this access-triggered cost. Deferred can avoid charging an access only when no request arrives until after a deadline-fired window has completed.
- **Refined conclusion:** for this workload with inter-burst idle shorter than tREFI, deferred scheduling is strictly worse than fixed and cannot hide anything; its phase-insensitivity is a liability, not a robustness win. Long idle gaps can let a deferred deadline-fired window complete without charging an access, but fixed scheduling already fires during idle in this no-queue model. A throughput advantage for adaptive postponement therefore remains unproven until request queues/arbitration expose work that can be scheduled around refresh.
...
make test-dram-refresh-debt-sweep      # 9 rows; deferred nominal-grid/debt semantics
...
Preserve all modes. `none` stays the zero/default compatibility path. Per-bank refresh is the physically realistic choice for a DDR5-era controller and costs ~1% vs ~15% service under steady traffic in this model, at the price of per-bank control state and 4× commands; all-bank remains a valid low-complexity controller. Fixed scheduling is the conservative default: the phase sweep shows it pays zero refresh stall on most phases of a burst-idle workload and its worst phase (+81%) is still far below deferred's +325–407%. Deferred preserves the nominal tREFI grid (late commands consume slack) and is valuable as a physically plausible latency/jitter alternative, but it has no demonstrated throughput advantage in the current no-queue model: fixed also fires during idle, while a post-schedule access makes deferred pay the lockout. Queue-aware postpone/pull-in remains blocked on request, arbitration, and refresh-credit contracts. Rate 2x/4x should be reserved for high-temperature retention requirements — it multiplies refresh cost linearly. Do not claim any mode is universally best or that this serialized model captures a production controller's overlap.
