...
`max_burst_bytes` remains the common compatibility setting. `read_max_burst_bytes` and `write_max_burst_bytes` are optional directional overrides; zero inherits the common value. All nonzero limits must be powers of two from 16 through 65,536 bytes. The existing common `burst_issue_cycles` remains direction-independent, so this exploration isolates geometry rather than conflating it with separate read/write command-pipeline costs.
...
- **Control complexity:** Symmetric mode permits one shared limit. Directional mode adds selection and independent protocol constraints; real split issue pipelines may need separate issue-cost knobs, which are not modeled here.
