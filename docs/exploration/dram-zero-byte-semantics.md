...
Before this change, every non-ideal zero-byte read/write paid base latency and changed request, row, direction, and channel-availability state. In `burst_span_credit`, an unaligned empty request also computed:
...
| Mode | Read zero bytes | Write zero bytes | Requests | Occupied bytes | Row misses |
|---|---:|---:|---:|---:|---:|
| NONE | 0 cycles | 0 cycles | 0 | 0 | 0 |
| FIXED | 0 | 0 | 0 | 0 | 0 |
| IDLE_CREDIT | 0 | 0 | 0 | 0 | 0 |
| BURST_CREDIT | 0 | 0 | 0 | 0 | 0 |
| BURST_ROUND_CREDIT | 0 | 0 | 0 | 0 | 0 |
| BURST_SPAN_CREDIT | 0 | 0 | 0 | 0 | 0 |
| One-byte span control | 30-cycle read | — | 1 | 64 | 1 |

All 12 zero-byte direction/mode rows also gate zero contention stall, unchanged open-row state, unchanged channel availability, unchanged direction history, and zero pending bytes. The one-byte control proves the gate does not suppress the smallest nonempty request: it pays 10 base + 20 activation cycles and occupies one 64 B granule.

## Gain versus sacrifice

- **Throughput and latency:** Empty calls now consume zero modeled service and cannot delay following work. This removes fictitious overhead; it is not a throughput optimization for real payloads.
...
- **Compiler/runtime:** Compilers may safely elide empty tiles. If they emit them, the service model no longer fabricates memory work, but command-level diagnostics remain a separate missing contract.

## Implementation and verification

- `tu_cmodel/memory/dram_model.c`: early zero-byte return before ideal/non-ideal accounting for reads and writes.
- `tests/test_dram.c`: focused state-invariance regression, first observed failing with `zero-byte read returned service`, then passing after implementation.
- `tests/test_dram_zero_byte_sweep.c`: 12 mode/direction gates plus a one-byte control.
- `Makefile` and `.gitignore`: focused sweep target and artifact lifecycle.

Executed:

```text
make test-dram                 # RED: 32/33; then GREEN: 33/33
make test-dram-zero-byte-sweep # 12 zero-byte rows + one-byte control
```
...
This establishes service-API semantics only. It does not define whether text ASM, packed ISA, command queue, DMA descriptors, or compiler lowering reject, count, retire, or signal a zero-length command. Those producer/consumer surfaces must be audited separately before claiming command-level behavior.
