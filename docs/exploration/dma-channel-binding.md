...
**Question:** When producers do not distribute descriptors across independent DMA queues, should software retain explicit placement, should hardware rotate submissions, or should hardware inspect live descriptor count or assigned bytes?
...
In this measured skewed independent-path regime, all automatic policies reduce batch completion versus explicit placement: round-robin and least-outstanding by 53.7%, and least-bytes by 49.9%. Least-outstanding moves the first two second-wave descriptors to idle paths and the third behind the nearly completed long transfer, producing a 613-cycle batch. Least-bytes refuses that third placement because channel 0 still carries the long descriptor's full 12 KiB accounting; it sends the descriptor to channel 1 and finishes at cycle 664. This makes assigned byte totals more balanced (12/12/8 KiB versus least-outstanding's 16/8/8 KiB) but predicts remaining service less accurately at cycle 307. The slower row is retained because byte-aware distribution is physically plausible and may help when submissions occur early in long transfers; it is not universally better.
...
- **Per-request latency:** Round-robin is deterministic but may queue behind a long transfer. Least-outstanding reacts to live descriptor counts, but count is not remaining-byte work. Least-bytes distinguishes unequal descriptors but this implementation counts an active descriptor at full size until retirement; it can therefore avoid a path that is about to become idle. Explicit placement can be best when the compiler knows dependencies, locality, or path affinity.
- **Area/resources:** Explicit needs no distributor. Round-robin adds a channel pointer/modulo selection. Least-outstanding adds active/queued count reads and comparators. Least-bytes additionally needs per-queue byte totals or descriptor walks in the cmodel and wider comparators. Physical area, timing, and storage are unquantified.
...
- **Control complexity:** Explicit is simplest. Round-robin requires deterministic state. The minimum policies require a combinational or pipelined reduction and an atomic accepted-submission update. Least-bytes also needs overflow-safe accounting and a contract for active versus remaining bytes.
...
`tu.dma.channel_binding` in JSON/YAML accepts `explicit`, `round_robin`, `least_outstanding`, or `least_bytes`. The path is YAML/JSON → generator/header constants → canonical `tu_config_t` → `tu_runtime_config_t` → `tu_init_with_config()` → `tu_dma_init_config_full()` → live submission. `tu_dma_init_config_policy()` remains a compatibility wrapper selecting explicit binding.

## Fidelity limits

The descriptor engine is deterministic and tick-driven. Independent channel mode permits concurrent abstract transfer service but does not prove replicated DRAM interfaces or SRAM ports. Neither dynamic policy predicts completion time: least-outstanding ignores size, while least-bytes ignores elapsed active service and per-transfer fixed/SRAM penalties. No mode uses deadlines, dependencies, locality, priority, energy, heterogeneous path rates, or destination contention. The model omits producer issue overhead, descriptor fetch, hardware critical path, queue backpressure replay, path affinity, migration, and host-thread concurrency. Consequently these results support keeping all four alternatives configurable, not selecting one physical design universally. A future remaining-service policy is blocked until the tick state exposes partial transfer progress rather than fabricating it from completion timestamps.
