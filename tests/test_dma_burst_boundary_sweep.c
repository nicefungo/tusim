/* DMA size-only versus SRAM-address-boundary burst exploration. */
...
typedef enum { LINEAR_ALIGNED, LINEAR_MISALIGNED, LINEAR_TAIL,
               STRIDED_2D, STRIDED_3D, SCATTER, GATHER } case_t;
...
static void init_engine(int mode, uint32_t channels, int binding) {
    tu_dma_init_config_boundary(
        ...
        64u, 64u, 64u, 3u, 0u, 0u, false, false,
    ...
    case STRIDED_2D:
        for (uint32_t r = 0; r < 4; r++)
            if (memcmp(dst + (dir == TU_DMA_DIR_HOST_TO_TU ? 48 : 0) + r * 64,
                       input + r * 64, 20) != 0) return -4;
        return 0;
    case STRIDED_3D:
        for (uint32_t d = 0; d < 2; d++)
            for (uint32_t r = 0; r < 3; r++)
                if (memcmp(dst + (dir == TU_DMA_DIR_HOST_TO_TU ? 48 : 0) + d * 256 + r * 64,
                           input + d * 256 + r * 64, 20) != 0) return -5;
        return 0;
    case SCATTER:
        for (uint32_t i = 0; i < 5; i++)
            if (memcmp(raw + gather_index[i], input + i * 4, 4) != 0) return -6;
        return 0;
    case GATHER:
        for (uint32_t i = 0; i < 5; i++)
            if (memcmp(output + i * 4, input + gather_index[i], 4) != 0) return -7;
        return 0;
    }
    return -8;
}
...
static int propagation_rejection_gate(void) {
    tu_config_t cfg;
    char err[192] = {0};
    if (tu_config_load_string("{\"tu\":{\"dma\":{\"burst_boundary_mode\":\"sram_address\"}}}",
    ...
    if (rt.dma_burst_boundary_mode != TU_DMA_CONFIG_BURST_BOUNDARY_SRAM_ADDRESS)
        return -3;
    tu_init_with_config(&rt);
    if (g_tu_dma.burst_boundary_mode != TU_DMA_BOUNDARY_SRAM_ADDRESS) return -4;
    ...
    init_engine(2, 1, TU_DMA_BIND_EXPLICIT);
...
static int compatibility_gate(void) {
    ...
    rt.sram_w_size = rt.sram_a_size = rt.sram_o_size = 4096;
    tu_init_with_config(&rt);
...
static int projection_reversal_gate(int mode, uint8_t expected) {
    tu_sram_region_t sram;
    tu_sram_init(&sram, SPACE, "dma-boundary-projection");
    sram.banks.bw_modeling = false;
    init_engine(mode, 2, TU_DMA_BIND_EXPLICIT);
    tu_dma_descriptor_t *misaligned = tu_dma_desc_create_linear(
        0, TU_DMA_DIR_HOST_TO_TU, &sram, 1, input, 1, 64);
    ...
    tu_dma_descriptor_t *probe = tu_dma_desc_create_linear(
        0, TU_DMA_DIR_HOST_TO_TU, &sram, 1024, input, 1, 16);
    if (!misaligned || !rows || !probe ||
        tu_dma_submit_desc(misaligned) == 0 || tu_dma_submit_desc(rows) == 0)
        return -1;
...
int main(void) {
    ...
    static const char *names[] = {"linear_aligned", "linear_misaligned", "linear_tail",
                                  "strided_2d", "strided_3d", "scatter", "gather"};
    static const uint64_t done[][2] = {{56,56},{56,60},{60,64},{67,83},{75,99},{71,91},{71,91}};
    static const uint64_t occupied[][2] = {{64,64},{64,96},{96,128},{128,256},{192,384},{160,320},{160,320}};
    static const uint32_t useful[] = {64,64,80,80,120,20,20};
    ...
    for (int c = 0; c < 7; c++) {
        ...
        for (int m = 0; m < 2; m++) {
            ...
            if (rc) { fprintf(stderr, "FAIL: %s/%d rc=%d\n", names[c], m, rc); return 2; }
            printf("%-18s %-12s %10lu %6u %8lu\n", names[c],
                   m ? "sram_address" : "size_only", (unsigned long)done[c][m],
                   useful[c], (unsigned long)occupied[c][m]);
        }
    }
    if (run_case(LINEAR_MISALIGNED, TU_DMA_BOUNDARY_SRAM_ADDRESS,
                 TU_DMA_DIR_TU_TO_HOST, 60, 96) ||
        run_case(STRIDED_2D, TU_DMA_BOUNDARY_SRAM_ADDRESS,
                 TU_DMA_DIR_TU_TO_HOST, 83, 256)) {
        fprintf(stderr, "FAIL: store direction gate\n"); return 3;
    }
    if (projection_reversal_gate(TU_DMA_BOUNDARY_SIZE_ONLY, 0) ||
        projection_reversal_gate(TU_DMA_BOUNDARY_SRAM_ADDRESS, 1)) {
        fprintf(stderr, "FAIL: projected binding reversal\n"); return 4;
    }
    printf("PASS: aligned/misaligned/tail, 2D/3D/scatter/gather, load/store, occupied bytes, live/queued timing, config/default/rejection\n");
    return 0;
}
