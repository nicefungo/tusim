...
void tu_dma_init_config_boundary_aging_policy_cap(bool async, uint32_t num_channels,
    ...
    if (aging_scope != TU_DMA_AGING_FROM_SUBMISSION &&
        aging_scope != TU_DMA_AGING_FROM_QUEUE_HEAD) {
        fprintf(stderr, "DMA: unsupported aging scope %d\n", aging_scope);
        memset(&g_tu_dma, 0, sizeof(g_tu_dma));
        return;
    }
    ...
    if (aging_max_boost > UINT8_MAX) {
        fprintf(stderr, "DMA: aging maximum boost must be in [0,255], got %u\n",
                aging_max_boost);
        memset(&g_tu_dma, 0, sizeof(g_tu_dma));
        return;
    }
    g_tu_dma.aging_max_boost = aging_max_boost;
    ...
    if (base_latency_scope != TU_DMA_BASE_PER_DESCRIPTOR &&
        base_latency_scope != TU_DMA_BASE_PER_LOGICAL_SEGMENT) {
        fprintf(stderr, "DMA: unsupported base latency scope %d\n",
                base_latency_scope);
        memset(&g_tu_dma, 0, sizeof(g_tu_dma));
        return;
    }
    ...
    if (read_max_burst_bytes == 0)
        read_max_burst_bytes = max_burst_bytes;
    if (write_max_burst_bytes == 0)
        write_max_burst_bytes = max_burst_bytes;
    if (max_burst_bytes < 16 || max_burst_bytes > 65536 ||
        (max_burst_bytes & (max_burst_bytes - 1u)) != 0 ||
        read_max_burst_bytes < 16 || read_max_burst_bytes > 65536 ||
        (read_max_burst_bytes & (read_max_burst_bytes - 1u)) != 0 ||
        write_max_burst_bytes < 16 || write_max_burst_bytes > 65536 ||
        (write_max_burst_bytes & (write_max_burst_bytes - 1u)) != 0 ||
        burst_issue_cycles > 1024 ||
        (read_issue_configured && read_burst_issue_cycles > 1024) ||
        (write_issue_configured && write_burst_issue_cycles > 1024)) {
        fprintf(stderr, "DMA: burst bytes must be powers of two in [16,65536] and issue cycles in [0,1024], got %u/%u/%u issue=%u/%u/%u\n",
                max_burst_bytes, read_max_burst_bytes,
                write_max_burst_bytes, burst_issue_cycles,
                read_burst_issue_cycles, write_burst_issue_cycles);
        memset(&g_tu_dma, 0, sizeof(g_tu_dma));
        return;
    }
    g_tu_dma.max_burst_bytes = max_burst_bytes;
    g_tu_dma.read_max_burst_bytes = read_max_burst_bytes;
    g_tu_dma.write_max_burst_bytes = write_max_burst_bytes;
    g_tu_dma.burst_issue_cycles = burst_issue_cycles;
    g_tu_dma.read_burst_issue_cycles = read_issue_configured ?
                                           read_burst_issue_cycles : burst_issue_cycles;
    g_tu_dma.write_burst_issue_cycles = write_issue_configured ?
                                            write_burst_issue_cycles : burst_issue_cycles;
...
void tu_dma_init_config_boundary_aging_policy(bool async, uint32_t num_channels,
    ...
    tu_dma_init_config_boundary_aging_policy_cap(
        ...
        aging_scope, aging_metric, aging_increment, 0u, aging_cycle_quantum,
...
void tu_dma_init_config_boundary(bool async, uint32_t num_channels,
    ...
    tu_dma_init_config_boundary_aging(
        async, num_channels, max_queue_depth, bus_mode, arb_policy,
        TU_DMA_AGING_FROM_SUBMISSION, binding_policy, bus_width_bits,
        read_latency_cycles, write_latency_cycles, max_burst_bytes,
        read_max_burst_bytes, write_max_burst_bytes, burst_issue_cycles,
        read_burst_issue_cycles, write_burst_issue_cycles,
        read_issue_configured, write_issue_configured, burst_segmentation,
...
void tu_dma_init_config_segmentation(bool async, uint32_t num_channels,
    ...
    tu_dma_init_config_base_scope(
        ...
        write_max_burst_bytes, burst_issue_cycles, read_burst_issue_cycles,
        write_burst_issue_cycles, read_issue_configured,
        write_issue_configured, burst_segmentation,
        TU_DMA_BASE_PER_DESCRIPTOR);
}
...
void tu_dma_init_config_directional_burst(bool async, uint32_t num_channels,
    ...
    tu_dma_init_config_directional_issue(
        ...
        binding_policy, bus_width_bits, read_latency_cycles,
        write_latency_cycles, max_burst_bytes, read_max_burst_bytes,
        write_max_burst_bytes, burst_issue_cycles, 0, 0, false, false);
}
...
void tu_dma_init_config_burst(bool async, uint32_t num_channels,
                              ...
                              uint32_t max_burst_bytes,
                              uint32_t burst_issue_cycles) {
    tu_dma_init_config_directional_burst(async, num_channels, max_queue_depth,
                                         bus_mode, arb_policy, binding_policy,
                                         bus_width_bits, read_latency_cycles,
                                         write_latency_cycles, max_burst_bytes,
                                         0, 0, burst_issue_cycles);
}
...
void tu_dma_init_config_arch(bool async, uint32_t num_channels,
                             ...
                             int arb_policy, int binding_policy,
                             uint32_t bus_width_bits) {
    tu_dma_init_config_timing(async, num_channels, max_queue_depth, bus_mode,
                              arb_policy, binding_policy, bus_width_bits,
...
bool tu_dma_desc_set_external_address(tu_dma_descriptor_t *desc,
                                      uint64_t address) {
    if (!desc || desc->direction == TU_DMA_DIR_TU_TO_TU) return false;
    desc->external_address = address;
    desc->external_address_valid = true;
    return true;
}
...
static uint64_t descriptor_logical_segment_count(
    ...
    case TU_DMA_XFER_STRIDED_2D:
        return desc->dims[0];
    case TU_DMA_XFER_STRIDED_3D:
        return (uint64_t)desc->dims[0] * desc->dims[1];
    case TU_DMA_XFER_SCATTER:
    case TU_DMA_XFER_GATHER:
        return desc->index_count;
    default:
        return 1;
    }
}
...
static void add_address_bounded_segment(uint64_t sram_address,
    ...
    while (bytes > 0) {
        ...
        if (bound_sram) {
            uint64_t room = sram_boundary_bytes -
                            sram_address % sram_boundary_bytes;
        ...
        *payload_cycles += ceil_div_u64(chunk, g_tu_dma.bus_width_bytes);
        sram_address += chunk;
        external_address += chunk;
        bytes -= chunk;
    }
}

/* Host virtual pointers are never interpreted as bus addresses. External
 * boundary modes consume only explicit descriptor metadata. Logical SRAM and
 * host discontinuities are retained because a bounded command cannot span
 * two rows or indexed elements. */
static void descriptor_address_bounded_totals(
    ...
    const uint32_t *external_strides =
        desc->direction == TU_DMA_DIR_HOST_TO_TU ? desc->src_strides :
                                                   desc->dst_strides;
    uint64_t external_base = desc->external_address;
    *burst_count = 0;
    *payload_cycles = 0;
    ...
    case TU_DMA_XFER_STRIDED_2D:
        for (uint64_t r = 0; r < desc->dims[0]; r++)
            add_address_bounded_segment(base + r * strides[0],
        ...
        for (uint64_t d = 0; d < desc->dims[0]; d++)
            for (uint64_t r = 0; r < desc->dims[1]; r++)
                add_address_bounded_segment(base + d * strides[1] + r * strides[0],
                                            external_base + d * external_strides[1] +
    ...
    case TU_DMA_XFER_GATHER:
        for (uint64_t i = 0; i < desc->index_count; i++)
            add_address_bounded_segment(desc->index_list[i],
...
static uint64_t descriptor_base_cycles(const tu_dma_descriptor_t *desc) {
    ...
    uint64_t count = g_tu_dma.base_latency_scope ==
                         TU_DMA_BASE_PER_LOGICAL_SEGMENT ?
                     descriptor_logical_segment_count(desc) :
                     1u; /* preserve historical zero-byte descriptor setup */
    return base * count;
}
...
static uint8_t descriptor_effective_priority(
    ...
    uint64_t boost;
    if (age > UINT64_MAX / g_tu_dma.aging_increment)
        boost = UINT64_MAX;
    else
        boost = age * g_tu_dma.aging_increment;
    if (g_tu_dma.aging_max_boost != 0 &&
        boost > g_tu_dma.aging_max_boost)
        boost = g_tu_dma.aging_max_boost;
    if (boost > room)
        boost = room;
