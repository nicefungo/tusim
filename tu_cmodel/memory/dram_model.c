...
static uint64_t explicit_row_penalty(tu_dram_model_t *dram, uint64_t addr) {
    ...
    if (dram->row_policy == TU_DRAM_ROW_ADAPTIVE_TIMEOUT &&
        dram->open_rows[idx] != UINT64_MAX &&
        dram->row_last_access_cycle[idx] != UINT64_MAX &&
        dram->current_cycle - dram->row_last_access_cycle[idx] >
            dram->row_open_timeout_cycles) {
        dram->open_rows[idx] = UINT64_MAX;
        dram->stats.total_row_timeout_precharges++;
    }
    if (dram->open_rows[idx] == row) {
        dram->stats.total_row_hits++;
        dram->row_last_access_cycle[idx] = dram->current_cycle;
        return 0;
    }
    dram->stats.total_row_conflicts++;
    uint64_t penalty;
...
/* Bytes charged to the serialized channel and coarse bandwidth window.
 * ROUND rounds payload size and assumes aligned requests; SPAN counts every
 * address-aligned protocol burst touched. Exact-byte modes preserve payload. */
static uint64_t occupied_bytes_for(const tu_dram_model_t *dram, uint64_t addr,
    ...
    uint64_t span = num_bytes;
    if (dram->turnaround_mode == TU_DRAM_TURNAROUND_BURST_SPAN_CREDIT)
        span += addr & (burst - 1);
    return (span + burst - 1) / burst * burst;
}
...
void tu_dram_read(tu_dram_model_t *dram, uint64_t addr,
    ...
    uint64_t occupied_bytes = occupied_bytes_for(dram, addr, num_bytes, true);
     ...
     * Rounded modes charge complete size- or address-span protocol bursts. */
...
void tu_dram_write(tu_dram_model_t *dram, uint64_t addr,
    ...
    uint64_t occupied_bytes = occupied_bytes_for(dram, addr, num_bytes, false);
    uint64_t consumed_bw = dram->pending_read_bytes + dram->pending_write_bytes +
                           occupied_bytes;
    if (consumed_bw > dram->bandwidth_available) {
        stall += dram->bw_window_size_cycles -
...
bool tu_dram_set_row_policy_timing(tu_dram_model_t *dram,
    ...
    return tu_dram_set_row_policy_timeout(dram, policy, activate_penalty_cycles,
                                          conflict_penalty_cycles, 0);
}
...
bool tu_dram_set_turnaround(tu_dram_model_t *dram,
    ...
    if (!dram || mode < TU_DRAM_TURNAROUND_NONE ||
        mode > TU_DRAM_TURNAROUND_BURST_SPAN_CREDIT ||
